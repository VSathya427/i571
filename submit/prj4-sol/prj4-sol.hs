...
categoryItems :: [OrderItem] -> Category -> [OrderItem]
categoryItems [] _ = []
categoryItems (x:xs) c
  | itemCategory x == c = x : categoryItems xs c
  | otherwise = categoryItems xs c
...
comprCategoryItems :: [OrderItem] -> Category -> [OrderItem]
comprCategoryItems xs c = [x | x <- xs, itemCategory x == c]
...
itemsTotal :: [OrderItem] -> Float
itemsTotal xs = sum [fromIntegral (itemNUnits x) * itemUnitPrice x | x <- xs]
...
substTerm (Var varName) bindings =
  case lookup varName bindings of
    ...
    Nothing -> Var varName
substTerm (Struct name terms) bindings =
  Struct name (map (\term -> substTerm term bindings) terms)
...
normalizeBindings :: [Binding] -> [Binding]
normalizeBindings bindings = sortBy compareBindings normalizedBindings
  where
    normalizedBindings = [(name, substTerm term bindings) | (name, term) <- bindings]
    compareBindings (name1, _) (name2, _) = compare name1 name2
...
unifyWithBindings t1 t2 b =
  case (t1, t2) of
    ...
    (Var x, _) ->
      case lookup x b of
        Just t' -> unifyWithBindings t' t2 b
        Nothing -> Just $ normalizeBindings ((x, t2) : b)
    (_, Var y) -> unifyWithBindings t2 t1 b
...
unifyTerms (t1:ts1) (t2:ts2) b =
  case unifyWithBindings t1 t2 b of
