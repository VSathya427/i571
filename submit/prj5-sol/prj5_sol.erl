...
category_items1(Category, [Item|Items]) ->
    case Item#order_item.category == Category of
        true -> [Item|category_items1(Category, Items)];
        false -> category_items1(Category, Items)
    end.
...
category_items3(_Category, Items) ->
    [Item || Item <- Items, Item#order_item.category == _Category].
...
delete_item(Sku, [Item | Rest]) ->
  case Item#order_item.sku == Sku of
    true -> delete_item(Sku, Rest);
    false -> [Item | delete_item(Sku, Rest)]
  end.
...
upsert_item(Item, [H | T]) ->
    case Item#order_item.sku == H#order_item.sku of
        true -> [Item | T];
        false -> [H | upsert_item(Item, T)]
    end.
...
items_req({read, Sku}, Items) ->
    case find_items([item_has_sku(Sku)], Items) of
        ...
        _ -> {err, io_lib:format("Unable to find item with SKU ~p", [Sku]), Items}
    end;
...
items_req_with_sort({sort}, Items) ->
    SortedItems = lists:sort(fun(I1, I2) -> I1#order_item.sku =< I2#order_item.sku end, Items),
    {ok, void, SortedItems};
items_req_with_sort(Req, Items) ->
    items_req(Req, Items).
...
start_items_server(Items, Fn) ->
    ServerPid = spawn(fun() -> items_server(Items, Fn) end),
    register(items, ServerPid),
    ServerPid.
...
items_server(Items, Fn) ->
    receive
        ...
        {From, {new_fn, NewFn}} ->
            From ! {self(), {ok, void}},
            items_server(Items, NewFn);
        {From, Req} ->
            {Status, Result, NewItems} = Fn(Req, Items),
            From ! {self(), {Status, Result}},
            items_server(NewItems, Fn)
    end.
